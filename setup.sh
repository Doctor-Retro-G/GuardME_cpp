...
# macOS: Check for Xcode Command Line Tools first
if [[ "$OSTYPE" == "darwin"* ]]; then
    if ! xcode-select -p &> /dev/null; then
        echo -e "       ${YELLOW}Xcode Command Line Tools not found${NC}"
        echo ""
        read -p "       Install Xcode Command Line Tools? (required for compiling) (y/n): " install_xcode
        if [[ "$install_xcode" =~ ^[Yy]$ ]]; then
            echo -e "       ${CYAN}Installing Xcode Command Line Tools...${NC}"
            echo -e "       ${YELLOW}A dialog will appear - click 'Install' to continue${NC}"
            xcode-select --install
            
            # Wait for installation to complete
            echo -e "       ${YELLOW}Waiting for Xcode Command Line Tools installation...${NC}"
            echo -e "       ${YELLOW}Press Enter after the installation completes${NC}"
            read -p ""
            ...
            echo -e "       ${RED}✗${NC} Xcode Command Line Tools are required to compile"
            echo -e "       ${YELLOW}Install manually with: xcode-select --install${NC}"
            exit 1
        fi
    else
...
if [[ "$OSTYPE" == "darwin"* ]] && [ "$PKG_MANAGER" == "unknown" ]; then
    ...
    echo ""
    read -p "       Install Homebrew? (recommended for managing dependencies) (y/n): " install_brew
    if [[ "$install_brew" =~ ^[Yy]$ ]]; then
        ...
        echo -e "       ${RED}✗${NC} Homebrew is required to install dependencies on macOS"
        exit 1
    fi
fi
